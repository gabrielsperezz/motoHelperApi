...
class EmpresaController
{
    ...
    public function getFormEmpresa(Application $app, $id)
    {
        ...
        if ($idValido && $empresasExistem) {
            ...
            $response = $app['twig']->render('admin/empresa/empresa_form.html.twig', $params);
        } elseif ($idValido && !$empresasExistem) {
            $response = $app['twig']->render('admin/errors/404.html.twig');
        } else {
            $params = [
                'empresa' => null
            ];
    ...
    {
        $response = $app['twig']->render('admin/empresa/empresa_form_busca.html.twig', ["empresas" => $this->getAllEmpresas($app)]);
        return $response;
    }
